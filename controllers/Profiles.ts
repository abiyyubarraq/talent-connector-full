...
export async function updateProfileHandler(req: Request, res: Response) {
  ...
  const updatedProfile = await findAndUpdate({ userId }, profileField, { new: true })
...
export async function updateExperience(req: Request, res: Response) {
  try {
    ...
    if (!profile) return res.status(400).json({ msg: "Profile not found" })
    const updatedProfile = await findAndUpdate({ userId }, { $push: { experience: req.body.experience } }, { new: true })
...
export async function updateEducation(req: Request, res: Response) {
  try {
    ...
    if (!profile) return res.status(400).json({ msg: "Profile not found" })
    const updatedProfile = await findAndUpdate({ userId }, { $push: { education: req.body.education } }, { new: true })
    return res.json(updatedProfile)
  } catch (e: any) {
    Logging.error(e)
    return res.status(500).send("server error")
...
export async function deleteExperience(req: Request, res: Response) {
  try {
    ...
    if (!profile) return res.status(400).json({ msg: "Profile not found" })
    const updatedProfile = await findAndUpdate({ userId }, { $pull: { experience: { _id: expId } } }, { new: true })
    return res.json(updatedProfile)
  } catch (e: any) {
    Logging.error(e)
    return res.status(500).send("server error")
...
export async function deleteEducation(req: Request, res: Response) {
  try {
    ...
    if (!profile) return res.status(400).json({ msg: "Profile not found" })
    const updatedProfile = await findAndUpdate({ userId }, { $pull: { education: { _id: eduId } } }, { new: true })
    return res.json(updatedProfile)
  } catch (e: any) {
    Logging.error(e)
    return res.status(500).send("server error")
